...
class App {
    ...
    public app_instance: Express | http.Server = express();
    ...
    constructor(extensions: Extension[]) {
        extensions.forEach(extension => {
            switch(extension[1]) {
                case 'Middleware':
                    (this.app_instance as Express).use(extension[0]);
                    break;
                case 'Route':
                    (this.app_instance as Express).use(extension[0][0], extension[0][1]);
                    break;
                case 'Option':
                    (this.app_instance as Express).options(extension[0][0], extension[0][1]);
                    break;
                case 'Skip':
                    break;
            }
        });
        this.app_instance = this.app_instance.listen(process.env.port as unknown as number, process.env.hostname as string, () => {
